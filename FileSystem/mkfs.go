...
func Mkfs(id string, type_ string, fs_ string) {
	...
	//----------Recupera los datos del disco (no se valido si existe o no)
	// Open bin file
	...
	//----------Recupera la particion que menciona el id
	var index int = -1
	...
	if index != -1 {
		fmt.Println("Encontro particion")
	} else {
		fmt.Println("Partition not found")
		return
	}

	//-------------Realizacion de calculos (primeras paginas de enunciado) -> ya despejo n codiogo c++
...
func create_ext2(n int32, partition Structs.Partition, newSuperblock Structs.Superblock, date string, file *os.File) {
	...
	//TRATAR DE HACER ESTAS ESCRITURA 1024
