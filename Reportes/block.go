...
func InitBloques(disco string, id string) string {
	text := ""
	indexI = 0
	indexB = 0
	...
	text += RecorridoBloques(Inode0, file, tempSuperblock)

	return text
}

func RecorridoBloques(Inode Structs.Inode, file *os.File, tempSuperbloque Structs.Superblock) string {
	text := ""
	//PARA INODOS
				...
				if i < 12 { //Pendiente verificar el indice
					...
					for _, folder1 := range crrFolderBlock.B_content {
						...
						if nombre_docu != "." && nombre_docu != ".." && nombre_docu != "" {
							...
							text += RecorridoBloques(NextInode, file, tempSuperbloque)
